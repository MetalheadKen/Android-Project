...
public class IBeaconService {
  ...
  //  double [][] beacon_distances = new double[MAX_DEVICE_COUNT][10];
 //   int [] beacon_distances_index = new int[MAX_DEVICE_COUNT];
   ...
   /*     int index = deviceMacList.indexOf(device.getAddress());
        int distance_index = beacon_distances_index[index] % 10;

        beacon_distances[index][distance_index] = getDistance(txpower, rssi_filter);
        beacon_distances_index[index]++;

        Arrays.sort(beacon_distances[index]);

        double sum = 0.0;
        for(int i = 0; i < 10; i++) {
            if(i >= 2 && i <= 7) {
                sum += beacon_distances[index][i];
            }
        }*/

        return TraingularPosition(device, String.format("%.5f", getDistance(txpower, rssi_filter)), major & 0x0FFF, minor & 0x0FFF);
    }
    ...
    private int [] TraingularPosition(BluetoothDevice device, String distances, int x_point, int y_point) {
        ...
        coordinate[0] = ((int) (min_coordinate[0][X_COORDINATE] + min_coordinate[1][X_COORDINATE] + min_coordinate[2][X_COORDINATE]) / 3);
        coordinate[1] = ((int) (min_coordinate[0][Y_COORDINATE] + min_coordinate[1][Y_COORDINATE] + min_coordinate[2][Y_COORDINATE]) / 3);
    ...
    public String[] GetProductInformation(BluetoothDevice device, String[] uuid_check) {
        ...
        for (int i = 0; i < uuid_check.length; i++) {
            if (btDevice.get("uuid").equals(uuid_check[i])) {
                IBeaconGlobal.all_product_uuid[i] = "Have been used";
    ...
    public int GetBluetoothDeviceDetail(BluetoothDevice device, int rssi, byte[] scanRecord) {
        ...
        btmap.put("distance", String.format("%.5f", getDistance(scanRecord[check], rssi_filter)));
   ...
   //     if (true) {
     ...
     //   }
       ...
       /* Arrays.sort(beacon_rssi[index]);

        int sum = 0;
        for(int i = 0; i < MAX_STORE_RSSI_COUNT; i++) {
            if(i > 4 && i < 25) {
                sum += beacon_rssi[index][i];
            }
        }*/
